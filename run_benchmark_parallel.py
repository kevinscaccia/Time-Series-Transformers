...
manager = Manager()
metrics_table = manager.dict()
# proc shared table
metrics_table['serie_id'] = []
metrics_table['smape'] = []
metrics_table['mase'] = []


def processBatch(generated_tuple):
    train_serie, test_serie, serie_id, fh, freq, serie_sp = generated_tuple
    scaler = MinMaxScaler((-1,1))
...
with mlflow.start_run(run_name=f'{run_sp}'):
    ...
    threadPool.map(processBatch, m4_data.generate(random=False))
    threadPool.close()
    threadPool.join()
    metrics_dict = {
